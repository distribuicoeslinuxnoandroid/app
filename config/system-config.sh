...
show_progress_dialog apt-labeled 68 \
    ...
    "${label_install_script_download}" 'sudo apt-get apt-utils -y' \
    ...
    "${label_install_script_download}" 'sudo sh -c 'echo "deb [arch=arm64,armhf signed-by=/etc/apt/keyrings/packages.microsoft.gpg] https://packages.microsoft.com/repos/code stable main" > /etc/apt/sources.list.d/vscode.list'' \
    ...
    "${label_install_script_download}" 'cd zorin-icon-themes/' \
    "${label_install_script_download}" 'mv Zorin*/ /usr/share/icons/' \
    "${label_install_script_download}" 'cd $HOME' \
    "${label_install_script_download}" 'cd zorin-desktop-themes/' \
    "${label_install_script_download}" 'mv Zorin*/ /usr/share/themes/' \
    "${label_install_script_download}" 'cd $HOME' \
    "${label_install_script_download}" 'rm -rf zorin-*-themes/' \
    "${label_install_script_download}" 'sudo install -d -m 0755 /etc/apt/keyrings' \
    "${label_install_script_download}" 'wget -q https://packages.mozilla.org/apt/repo-signing-key.gpg -O- | sudo tee /etc/apt/keyrings/packages.mozilla.org.asc' \
    ...
    "${label_install_script_download}" 'sed -i 's|Exec=/usr/share/code/code|Exec=/usr/share/code/code --no-sandbox|' /usr/share/applications/code*.desktop' \
